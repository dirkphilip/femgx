...
import { cross, length, normalize, scale, subtract, type Vec3 } from "../math/vec3";
...
export class CameraTransition {
  ...
  start(initial: Camera, target: Camera, durationMs: number, update: CameraTransitionUpdate): void {
  ...
  private tick(active: ActiveTransition, time: number): void {
    ...
    active.update(
      complete
        ? active.target
        : interpolateCamera(active.initial, active.target, easeOutCubic(linear)),
      complete,
    );
    if (complete) {
      this.active = undefined;
