...
| Screen-space box-selection gesture/event shell (primary mouse/pen drag lifecycle + typed events)                                   | interaction        | same       | **Core now** | Reusable renderer-independent box-drag lifecycle in `installBoxSelection`; coordinates are clamped canvas-local CSS pixels. Emits start/change/complete/cancel only — never mutates selection. `FemViewport.pickRegion` adds visible-intersection target discovery through the cached GPU ID snapshot; selection policy remains host-owned. |
