...
## Topology transfer and GPU residency contracts
...
| Contract                          | Client owns                                                | GPU owns                                                                               | Local interior reveal                                                                           | Status       |
| --------------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ------------ |
| Host-reduced surface              | Only the complete display-relevant `surfacePart()` payload | Only retained facets, lines, and points                                                | No. Hiding retained geometry may leave a hole; omitted topology cannot be rendered or selected. | **Core now** |
| Client-full, GPU-surface-resident | Complete `ElementModel` topology                           | Only the current exterior and exposed interfaces                                       | Intended to be local after a bounded skin rebuild/upload, but not currently implemented.        | **Deferred** |
| Fully resident model              | Complete `ElementModel` topology                           | Complete reusable geometry; an optional compact face subset limits ordinary submission | Yes, because the client and renderer retain the relevant faces and identities.                  | **Core now** |

The first contract is the client/server transfer-minimizing path. The server may
retain the omitted volume and the host may later replace the part or scene with
a different complete payload, but femgx does not own the server protocol,
incremental requests, caching, or progressive refinement. Picking, results,
deformation, visibility, bounds, and selection apply only to identities present
in the current client payload.

The middle contract is a GPU-memory optimization, not a transport optimization:
the CPU still needs the complete topology required to derive newly exposed
faces. It must not be approximated by silently discarding topology or by drawing
all retained interior faces whenever one element is hidden. Before it becomes
Core, its design must define per-occurrence visibility with reusable instanced
parts, update latency, buffer ownership, and peak/steady GPU memory budgets. The
recommended adaptive design is recorded in
...
and interact with interior faces. At present, any hidden body, block, or element
