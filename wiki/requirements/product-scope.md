...
| GPU picking (element + node strict; face Core) with host-mappable ids                                                              | viewport + picking | same       | **Core now** | Interaction picking is `FemViewport.pick` → `resolvePickTarget`. Node and element ids are strict product requirements; face is Core. Multi-hit `pickMany` is future (below), not Core-now.                                                                                          |
...
| Screen-space box-selection gesture/event shell (primary mouse/pen drag lifecycle + typed events)                                   | interaction        | same       | **Core now** | Reusable renderer-independent box-drag lifecycle in `installBoxSelection`; coordinates are clamped canvas-local CSS pixels. Emits start/change/complete/cancel only — never picks, mutates selection, or discovers targets. GPU multi-target region picking stays deferred (below). |
...
## Future: GPU region target discovery (`pickMany`)

Not Core-now. The screen-space box-selection gesture/event shell is accepted
core interaction infrastructure; the demo previews the live rectangle but
deliberately stops before model targets. A later extension can expose unique
targets under a screen region via ID-buffer readback (preferred first shape for
box-ish pick lists). Ordered multi-hit along a ray (depth peeling / A-buffer) is
a harder follow-on and must pass the [[#decision-gate|decision gate]]
separately. Do not grow a parallel CPU pick-list path.
