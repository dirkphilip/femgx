...
| Authored elemental orientation frames                                                                                                                                                   | renderer + results   | viewport   | **Core now** | A dense part-local `ElementFrameField` stores one authored X/Y/Z frame per element row. The renderer extends the orientation path with one non-pickable RGB triad per complete frame, anchored to the deformed element and preserving instancing, visibility, section clipping, depth-visible/weighted-ghost presentation, and recovery. Missing rows are `NaN`; frames are shared by every occurrence of the reusable part.                                                                                                                                                                                                                                                                  |
| Authored nodal loads (forces and moments)                                                                                                                                               | renderer + results   | viewport   | **Core now** | A dense part-local `NodalLoadField` stores six authored components per node: force `Fx/Fy/Fz` and moment `Mx/My/Mz`. Complete force vectors render as straight non-pickable arrows; complete moment vectors render as curved non-pickable arrows. `forceLengthScale` is part-local length per force unit and `momentLengthScale` is part-local radius per moment unit. Loads anchor only to authored local nodes, are shared by every occurrence, and preserve transforms, deformation, visibility, section clipping, depth/ghost, and recovery. Mixed finite/missing triplets are rejected; all-`NaN` triplets are missing. Occurrence-specific overrides remain a copy-part responsibility. |
...
subsystem. Frames are shared by placements just like part geometry, so forces,
loads, occurrence-specific overrides, custom glyph plugins, derived frames, and
frame editing remain out of scope. Hosts that need occurrence-specific data may
currently copy a part definition. The explicit `ElementFrameField` symbol is
