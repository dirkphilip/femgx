...
| FE node annotations                                                                                                                | renderer + demo           | same       | **Core now** | Center-depth classification hides occluded nodes and draws complete 6 CSS-px front glyphs (DPR-scaled) without zoom-dependent depth offsets.                                                              |
...
- Flat `compileScene` snapshot and CPU raycast stack (`createPickScene` /
  ...
  `renderer.pick`.
- Quadratic elements and adjacency UI polish remain in the codebase as
  **Deferred**; they must not grow, and a future issue may trim them behind an
