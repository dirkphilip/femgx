...
| FE node annotations                                                                                                                | renderer + demo           | same       | **Core now** | Node inspection must remain readable over element faces, including occluded nodes; the annotation pass is an x-ray overlay.                                                                               |
...
| IO: VTU, Gmsh, Abaqus adapters, chunked model builder, cancellation/progress                                                       | ~2.8k                     | ~1.0k      | **Deferred** | Extra formats and streaming-style builders are breadth, not minimum product.                                                                                                                              |
| Large-model streaming (spatial partitioning, LOD, upload budgets, worker parsing, coordinate rebasing)                             | ~0.8k                     | ~0.7k      | **Deferred** | In-memory models are the minimum product; chunked multi-hundred-million-element loading is a future capability.                                                                                           |
...
element/face/node ids (node and element strict; face Core), readable x-ray
node annotations, selection/
