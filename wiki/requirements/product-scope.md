...
| Authored elemental orientation frames                                                                                                                                                   | renderer + results   | viewport   | **Core now** | A dense part-local `ElementFrameField` stores one authored X/Y/Z frame per element row. The renderer extends the orientation path with one non-pickable RGB triad per complete frame, anchored to the deformed element and preserving instancing, visibility, section clipping, depth-visible/weighted-ghost presentation, and recovery. Missing rows are `NaN`; frames are shared by every occurrence of the reusable part. Forces/loads, occurrence-specific result overrides, custom glyph plugins, and derived frames remain deferred; copying a part is the current occurrence-specific workaround. |
