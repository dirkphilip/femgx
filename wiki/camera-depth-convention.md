...
- `src/camera/project-polygon.ts` `projectPolygon`, which clips a world-space
  ...
  points. This is what the CPU fallback demos use so faces straddling the
  camera plane render instead of being dropped whole.
...
- The CPU fallback 2D demos dropped a whole projected face when any vertex had
  `clip.w <= 0`; they now clip the polygon against the camera plane.
