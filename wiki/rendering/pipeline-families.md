...
| -------------------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| Instanced triangle, expanded line, and point glyph | [Primitive pipelines](../../src/renderer/shaders/primitive-pipelines.ts) | `primitiveVariants` owns the role-specific vertex program, entry point, vertex layout, topology, culling, and default depth comparison.                                              | Opaque color, weighted transparency, four-target picking, and dense-selection triangle depth bias.                |
...
| Selection geometry                                 | [Selection pipelines](../../src/renderer/shaders/selection-pipelines.ts) | Each specialized selection vertex program is paired with its required layout and primitive state; procedural node variants consume the node-sprite contract instead of restating it. | Depth-visible color and occluded weighted ghost; expanded and compact node orderings are named geometry variants. |
...
The instanced primitive and selection owners still use distinct vertex modules
and binding contracts. Their topology/layout overlap is a known broad migration
boundary: changes must audit both owners until a dedicated shared family spec
can represent those role-specific vertex programs without a boolean matrix.
That migration is not required for the node-sprite regression.
