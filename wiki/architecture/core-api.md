...
| Area        | Core API                                                                                                                                                                                                        | Owns                                                                                                                                                                                                                                      |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Geometry    | `Geometry`, `Part`, `createPart`, `GeometryBody`, `GeometryElementBlock`, `FaceSubset`, `polygonGeometry`, `polygonPart`                                                                                        | Validated immutable local positions, indices, derived body/block/element/node/face metadata, and derived local bounds. Focused validators and raw bound calculation are implementation helpers.                                           |
| Elements    | `createElement`, `ElementModel`, `ElementBlock`, `Body`, `createElementModel`, `ElementModelValidationError`, `heterogeneousElementParts`, `boundaryFaceRefs`, `FaceIdRef`, `ElementShape`                      | Validated FE connectivity (Point, Line, Line3, Triangle, Tri6, Quad, Quad8, Tet4, Tet10, Wedge6, Pyramid5, Hex8, Hex20), optional semantic block/body ownership, canonical topology, mixed primitive grouping, and face selection inputs. |
...
- `ElementBlockId` and `BodyId` are model-local one-based identities. A model
  block groups non-overlapping element membership; a body either groups direct
  elements or aggregates blocks. Derived geometry flattens block-defined bodies
