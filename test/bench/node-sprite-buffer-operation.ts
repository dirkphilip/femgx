...
/** Measures cold allocation and expansion of the large fixture's node sprites. */
export function nodeSpriteBufferOperation(fixture: NodeSelectionFixture): OperationSpec {
  ...
  return {
    ...
    workloadUnit: "authored node centers expanded into four-vertex sprites",
    ...
    workloadDetails: {
      ...
      outputPositionBytes: pickIds.length * 12 * Float32Array.BYTES_PER_ELEMENT,
      outputIdBytes: pickIds.length * 4 * Uint32Array.BYTES_PER_ELEMENT,
      outputIndexBytes: pickIds.length * 6 * Uint32Array.BYTES_PER_ELEMENT,
    },
    run: () => {
      assertNodeSpriteBuffers(buildNodeSpriteBuffers(positions, pickIds), pickIds);
    },
  };
}
...
function assertNodeSpriteBuffers(
  ...
  if (
    buffers.positions.length !== pickIds.length * 12 ||
    buffers.ids.length !== pickIds.length * 4 ||
