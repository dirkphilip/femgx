...
describe("through-box local bounds", () => {
  it.each([
    ["wholly inside", new Float64Array([-0.5, -0.5, -0.5, 0.5, 0.5, 0.5]), true],
    ["wholly outside", new Float64Array([2, -0.5, -0.5, 3, 0.5, 0.5]), false],
    ["crossing", new Float64Array([-2, -0.5, -0.5, 0.5, 0.5, 0.5]), undefined],
  ...
  it("transforms world planes for translated and scaled affine occurrences", () => {
    ...
    expect(planes).toBeDefined();
    expect(
      classifyLocalBounds(new Float64Array([-0.5, -0.5, -0.5, 0.5, 0.5, 0.5]), 0, planes ?? [], 0),
    ).toBe(false);
    ...
    expect(
      classifyLocalBounds(
        new Float64Array([-0.25, -0.25, -0.25, 0.25, 0.25, 0.25]),
        0,
        scaled ?? [],
        0,
      ),
  ...
  it("keeps section-plane crossings on the exact primitive path", () => {
    ...
    expect(
      classifyLocalBounds(
        new Float64Array([-0.75, -0.25, -0.25, 0.75, 0.25, 0.25]),
        0,
        planes ?? [],
        0,
      ),
