...
function main() {
  ...
  try {
    ...
    const smokeTs = [
      ...
      "    { kind: 'part', placementId: 'typed-part', partId: typedPart.id, transform: identityMatrix() },",
      "  ] })",
      "  .setRootAssembly(2)",
      ".build();",
      "const scene = createSceneBuilder()",
      ...
      'const bodyTarget: InteractionTarget = { kind: "body", partOccurrenceId: "1/first", bodyId: 0 };',
      "let interaction = createInteractionState();",
      "const partStyle: StyleOverride = { lineWidthPixels: 2 };",
      "interaction = setPartOverride(interaction, part.id, partStyle);",
      'interaction = setPartOccurrenceOverride(interaction, "1/first", { lineWidthPixels: 3 });',
      'interaction = setPartOccurrenceOverrides(interaction, [["1/first", { emissive: 0.2 }]]);',
