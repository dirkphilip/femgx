...
function main() {
  ...
  try {
    ...
    const smokeTs = [
      'import { boxSelectionFrustum, createViewport, createInteractionState, createPart, createResultField, createScene, identity, setInstanceOverride, setPartOverride, setTargetHighlighted, setTargetSelected, translation, UnknownSceneIdentityError, type Viewport, type InteractionTarget, type StyleOverride } from "femgx";',
...
function checkBundleBudgets(root) {
  ...
  for (const entry of entries) {
    ...
    if (entry === "femgx") {
      ...
      expect(rawBytes <= 445_000, `root bundle exceeds raw budget: ${rawBytes}`);
